...
import { Pill, Loader2, Download, FileText, AlertTriangle, CheckCircle2, Beaker, Layers, Languages } from 'lucide-react';
...
import { buildFinalRows, type FinalRow, type UnmatchedRow, type MFDSCandidate, type MatchedResult } from '@/lib/drug-matcher';
import { processProducts, translateEngToKor, getUniqueKeys } from '@/lib/process-engine';
import { getMFDSData } from '@/lib/mfds-local';

const SUPABASE_URL = import.meta.env.VITE_SUPABASE_URL as string;
const SUPABASE_KEY = import.meta.env.VITE_SUPABASE_PUBLISHABLE_KEY as string;

type AppPhase = 'idle' | 'loading-data' | 'translating' | 'review' | 'processing' | 'done';
...
const Index = () => {
  ...
  const processing = phase === 'processing' || phase === 'translating' || phase === 'loading-data';
  const done = phase === 'done';
  const canProcess = inputRows.length > 0 && !processing && phase !== 'review';
      ...
      <div className="flex flex-1 overflow-hidden">
        <aside className="w-72 border-r border-border bg-card p-4 space-y-4 flex-shrink-0 overflow-y-auto">
          ...
          >
            ...
            {phase === 'loading-data' ? '데이터 로딩...' : phase === 'translating' ? '번역 중...' : processing ? '처리 중...' : '분석 실행'}
          </button>
          ...
          {phase === 'idle' && (
            <div className="flex items-center justify-center h-full">
              <div className="text-center text-muted-foreground space-y-2">
                ...
                <p className="text-sm">분석할 엑셀 파일을 업로드한 후 분석을 실행하세요.</p>
              </div>
            </div>
          )}
          ...
          {phase === 'loading-data' && (
            <div className="flex items-center justify-center h-full">
              <div className="text-center text-muted-foreground space-y-2">
                ...
                <p className="text-sm">MFDS 허가 데이터를 불러오고 있습니다...</p>
              </div>
            </div>
          )}
